...
int open_wintap(struct tuntap_dev *device, const char * address_mode, /* "static" or "dhcp" */
char *device_ip, char *device_mask, const char *device_mac, int mtu)
{
    ...
    if ((rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, NETWORK_CONNECTIONS_KEY, 0,
            KEY_READ, &key))) {
    ...
    for (i = 0;; i++) {
        ...
        _snprintf(regpath, sizeof(regpath), "%s\\%s\\Connection",
                NETWORK_CONNECTIONS_KEY, adapterid);
        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, (LPCSTR) regpath, 0, KEY_READ,
                &key2))
            continue;
        ...
        _snprintf(tapname, sizeof(tapname), USERMODEDEVICEDIR "%s" TAPSUFFIX,
                adapterid);
        device->device_handle = CreateFile(tapname,
                GENERIC_WRITE | GENERIC_READ, 0, /* Don't let other processes share or open
                 the resource until the handle's been closed */
                0, OPEN_EXISTING, FILE_ATTRIBUTE_SYSTEM | FILE_FLAG_OVERLAPPED,
                0);
    ...
    if (!DeviceIoControl(device->device_handle, TAP_IOCTL_GET_MAC,
            device->mac_addr, sizeof(device->mac_addr), device->mac_addr,
            sizeof(device->mac_addr), &len, 0)) {
    ...
    printf(
            "Open device [name=%s][ip=%s][ifName=%s][MTU=%d][mac=%02X:%02X:%02X:%02X:%02X:%02X]\n",
            device->device_name, device_ip, device->ifName, device->mtu,
            device->mac_addr[0] & 0xFF, device->mac_addr[1] & 0xFF,
            device->mac_addr[2] & 0xFF, device->mac_addr[3] & 0xFF,
            device->mac_addr[4] & 0xFF, device->mac_addr[5] & 0xFF);
    ...
    if (0 == strcmp("dhcp", address_mode)) {
        _snprintf(cmd, sizeof(cmd),
                "netsh interface ip set address \"%s\" dhcp", device->ifName);
    } else {
        _snprintf(cmd, sizeof(cmd),
...
int tuntap_read(struct tuntap_dev *tuntap, unsigned char *buf, int len)
{
    ...
    if (ReadFile(tuntap->device_handle, buf, len, &read_size,
            &tuntap->overlap_read)) {
    ...
    case ERROR_IO_PENDING:
        ...
        GetOverlappedResult(tuntap->device_handle, &tuntap->overlap_read,
                &read_size, FALSE);
        return read_size;
        break;
    default:
...
int tuntap_write(struct tuntap_dev *tuntap, unsigned char *buf, int len)
{
    ...
    if (WriteFile(tuntap->device_handle, buf, len, &write_size,
...
int tuntap_open(struct tuntap_dev *device, char *dev, const char *address_mode, /* static or dhcp */
char *device_ip, char *device_mask, const char * device_mac, int mtu)
{
    return (open_wintap(device, address_mode, device_ip, device_mask,
            device_mac, mtu));
}
