...
static int localip2addr(n2n_sock_t * l_ip, const n2n_local_ip_t local_ip_strIn,
        int local_port);

static void send_packet2net(n2n_edge_t * eee, uint8_t *decrypted_msg,
        size_t len);
...
static int readConfFile(const char * filename, char * const linebuffer)
{
    ...
    if (stat(filename, &stats)) {
        if (errno == ENOENT
            ...
            traceEvent(TRACE_ERROR,
                    "parameter file %s not found/unable to access\n", filename);
                    else
    ...
    while (fgets(buffer, MAX_CONFFILE_LINE_LENGTH, fd)) {
        ...
        if ((strlen(linebuffer) + strlen(buffer) + 2)
                < MAX_CMDLINE_BUFFER_LENGTH) {
...
static int edge_init_twofish(n2n_edge_t * eee, uint8_t *encrypt_pwd,
        uint32_t encrypt_pwd_len)
{
    return transop_twofish_setup(&(eee->transop[N2N_TRANSOP_TF_IDX]), 0,
            encrypt_pwd, encrypt_pwd_len);
}
...
static int n2n_tick_transop(n2n_edge_t * eee, time_t now)
{
    ...
    tst = (eee->transop[N2N_TRANSOP_NULL_IDX].tick)(
            &(eee->transop[N2N_TRANSOP_NULL_IDX]), now);
    tst = (eee->transop[N2N_TRANSOP_AESCBC_IDX].tick)(
            &(eee->transop[N2N_TRANSOP_AESCBC_IDX]), now);
    if (tst.can_tx) {
        traceEvent(TRACE_DEBUG, "can_tx AESCBC (idx=%u)", (unsigned int)N2N_TRANSOP_AESCBC_IDX);
        trop = N2N_TRANSOP_AESCBC_IDX;
    }

    tst = (eee->transop[N2N_TRANSOP_TF_IDX].tick)(
            &(eee->transop[N2N_TRANSOP_TF_IDX]), now);
    if (tst.can_tx) {
...
static void edge_deinit(n2n_edge_t * eee)
{
    ...
    (eee->transop[N2N_TRANSOP_TF_IDX].deinit)(
            &eee->transop[N2N_TRANSOP_TF_IDX]);
    (eee->transop[N2N_TRANSOP_NULL_IDX].deinit)(
            &eee->transop[N2N_TRANSOP_NULL_IDX]);
}
...
static void help()
{
    ...
    printf(
            "-M <mtu>                 | Specify n2n MTU of edge interface (default %d).\n",
            DEFAULT_MTU);
    printf(
...
static ssize_t sendto_sock(int fd, const void * buf, size_t len,
        const n2n_sock_t * dest)
{
    ...
    sent = sendto(fd, buf, len, 0/*flags*/, (struct sockaddr *)&peer_addr,
        ...
        const n2n_mac_t dstMac)
{
    ...
    traceEvent(TRACE_INFO, "send REGISTER %s", sock_to_cstr(sockbuf, remote_peer));

    sendto_sock(eee->udp_sock, pktbuf, idx, remote_peer);
}

/** Send a QUERY_PEER packet to the current supernode. */
static void send_query_peer(n2n_edge_t * eee, const n2n_mac_t dstMac)
{
    uint8_t pktbuf[N2N_PKT_BUF_SIZE];
    size_t idx;
    n2n_common_t cmn = { 0 };
...
static void send_register_super(n2n_edge_t * eee, const n2n_sock_t * supernode)
{
    ...
    traceEvent(TRACE_INFO, "send REGISTER_SUPER to %s", sock_to_cstr(sockbuf, supernode));

    sendto_sock(eee->udp_sock, pktbuf, idx, supernode);
}

/** Send a REGISTER_ACK packet to a peer edge. */
static void send_register_ack(n2n_edge_t * eee, const n2n_sock_t * remote_peer,
        const n2n_REGISTER_t * reg)
{
    ...
    traceEvent(TRACE_INFO, "send REGISTER_ACK %s", sock_to_cstr(sockbuf, remote_peer));
 ...
 */
static void send_deregister(n2n_edge_t * eee, n2n_sock_t * remote_peer)
{
...
static void update_peer_address(n2n_edge_t * eee, uint8_t from_supernode,
        const n2n_mac_t mac, const n2n_sock_t * peer, time_t when);
void check_peer(n2n_edge_t * eee, uint8_t from_supernode, const n2n_mac_t mac,
        const n2n_sock_t * peer);
void establish_connection(n2n_edge_t * eee, const n2n_mac_t mac);
void set_peer_operational(n2n_edge_t * eee, const n2n_mac_t mac,
        const n2n_sock_t * peer);
...
void establish_connection(n2n_edge_t * eee, const n2n_mac_t mac)
{
    ...
    if (NULL == scan) {
        ...
        traceEvent(TRACE_DEBUG, "=== new pending %s -> %s",
                macaddr_str(mac_buf, scan->mac_addr), sock_to_cstr(sockbuf, &scan->sock));
    ...
    if (0 != sock_equal(&(scan->sock), peer)) {
        if (0 == from_supernode) {
            traceEvent(TRACE_NORMAL, "Peer changed %s: %s -> %s",
                    macaddr_str( mac_buf, scan->mac_addr ), sock_to_cstr(sockbuf1, &(scan->sock)),
...
{
    if (eee->sn_wait &&
            (nowTime > (eee->last_register_req + (eee->holepunch_interval / 10)))) {
        /* fall through */
    ...
    if (0 == eee->sup_attempts) {
        ...
        traceEvent(TRACE_WARNING,
                "Supernode not responding - moving to %u of %u",
    ...
    traceEvent(TRACE_DEBUG,
            "Registering with supernode (%s) (attempts left %u)",
...
static int find_peer_destination(n2n_edge_t * eee, n2n_mac_t mac_address,
        n2n_sock_t *destination, peer_info_t **peer)
{
    peer_info_t tmp;
    peer_info_t* scan = NULL;
    ...
    traceEvent(TRACE_DEBUG,
            "Searching destination peer for MAC %02X:%02X:%02X:%02X:%02X:%02X",
        ...
        if (tryscan) {
            if (tryscan->num_sockets == 0) {
                ...
                traceEvent(TRACE_INFO, "retrying to register peer (%s) -> [%s]",
                        macaddr_str( mac_buf, mac_address), sock_to_cstr( sockbuf, &tryscan->sock));
    ...
    traceEvent(TRACE_DEBUG, "find_peer_address (%s) -> [%s]",
            macaddr_str(mac_buf, mac_address), sock_to_cstr(sockbuf, destination));
 ...
 *  address. */
static int send_PACKET(n2n_edge_t * eee, n2n_mac_t dstMac,
        const uint8_t * pktbuf, size_t pktlen, n2n_sock_t * destination)
{
    n2n_sock_str_t sockbuf;
...
static void send_packet2net(n2n_edge_t * eee, uint8_t *tap_pkt, size_t len)
{
    ...
    traceEvent(TRACE_DEBUG,
            "encoded PACKET header of size=%u transform %u (idx=%u)",
...
static int is_ethMulticast(const void * buf, size_t bufsize)
{
    ...
    if (bufsize >= sizeof(ether_hdr_t)) {
        ...
        if ((0x01 == eh.dhost[0]) && (0x00 == eh.dhost[1]) &&
            (0x5E == eh.dhost[2]) && (0 == (0x80 & eh.dhost[3]))) {
...
static void readFromTAPSocket(n2n_edge_t * eee) {
    ...
    else
    {
        ...
        if (eee->drop_multicast &&
           (is_ip6_discovery(eth_pkt, len) || is_ethMulticast(eth_pkt, len))) {
 ...
 *  encrypted. */
static int handle_PACKET(n2n_edge_t * eee, const n2n_common_t * cmn,
...
static void readFromMgmtSocket(n2n_edge_t * eee, int * keep_running)
{
    ...
    recvlen = recvfrom(eee->udp_mgmt_sock, udp_buf, N2N_PKT_BUF_SIZE,
            0/*flags*/, (struct sockaddr *) &sender_sock, (socklen_t*) &i);
    ...
    if (recvlen >= 4) {
        ...
        if (0 == memcmp(udp_buf, "help", 4)) {
            ...
            msg_len += snprintf(
                    (char *) (udp_buf + msg_len), (N2N_PKT_BUF_SIZE-msg_len),
    ...
    if (recvlen >= 12) {
        if (0 == memcmp(udp_buf, "mangle_peer ", 12)) {
            ...
            n_matched = sscanf((const char *) udp_buf,
                    "mangle_peer %x:%x:%x:%x:%x:%x %d.%d.%d.%d %d", mac,
                    mac + 1, mac + 2, mac + 3, mac + 4, mac + 5, ip, ip + 1,
                    ip + 2, ip + 3, &port);
            for (j = 0; j < 6; j++)
...
static void readFromIPSocket(n2n_edge_t * eee)
{
    ...
    /*if (0 == memcmp(cmn.community, eee->community_name, N2N_COMMUNITY_SIZE)) {
        ...
        case MSG_TYPE_REGISTER:
            ...
            if (memcmp(reg.dstMac, (eee->device.mac_addr), 6) == 0)
            {
        ...
        case MSG_TYPE_REGISTER_SUPER_ACK:
            if (eee->sn_wait) {
                ...
                if (memcmp( rsa.cookie, eee->last_cookie, N2N_COOKIE_SIZE) == 0) {
                    if (rsa.num_sn > 0) {
...
static void supernode2addr(n2n_sock_t * sn, const n2n_sn_name_t addrIn)
{
    ...
    if (supernode_host) {
        ...
        const struct addrinfo aihints =
                {0, PF_INET, 0, 0, 0, NULL, NULL, NULL};
        ...
        if (supernode_port)
            ...
            traceEvent(TRACE_WARNING,
                    "Bad supernode parameter (-l <host:port>) %s %s:%s",
        ...
        if (nameerr == 0) {
            ...
            if (ainfo && (PF_INET == ainfo->ai_family)) {
                ...
                traceEvent(TRACE_WARNING,
                        "Failed to resolve supernode IPv4 address for %s", supernode_host);
            }
            ...
            traceEvent(TRACE_WARNING,
                    "Failed to resolve supernode host %s, assuming numeric", supernode_host);
 ...
 */
static int localip2addr(n2n_sock_t * l_ip, const n2n_local_ip_t local_ip_str,
        int local_port)
{
    /* TODO: IPv6? */
    unsigned int ip[4];
    ...
    n_matches = sscanf(local_ip_str, "%u.%u.%u.%u", &ip[0], &ip[1], &ip[2], &ip[3]);
 ...
 */
static int scan_address(char * ip_addr, size_t addr_size, char * ip_mode,
        size_t mode_size, const char * s)
{
    int retval = -1;
    char * p;
...
int main(int argc, char* argv[])
{
    ...
    for (i = 1; i < argc; ++i) {
        if (argv[i][0] == '@') {
            if (readConfFile(&argv[i][1], linebuffer) < 0)
                exit(1); /* <<<<----- check */
        } else if ((strlen(linebuffer) + strlen(argv[i]) + 2)
                < MAX_CMDLINE_BUFFER_LENGTH)
                {
            strncat(linebuffer, " ", 1);
    ...
    while ((opt = getopt_long(effectiveargc, effectiveargv,
        ...
        {
            scan_address(ip_addr, N2N_NETMASK_STR_SIZE, ip_mode,
                         N2N_IF_MODE_SIZE, optarg);
            break;
        }
    ...
    (tuntap_dev_name[0] != 0) &&
#endif
        (eee.community_name[0] != 0) && (ip_addr[0] != 0))) {
            help();
    }
    ...
    if ((userid != 0) || (groupid != 0)) {
        traceEvent(TRACE_NORMAL,
                "Interface up. Dropping privileges to uid=%d, gid=%d",
    ...
    if (encrypt_key) {
        if (edge_init_twofish(&eee, (uint8_t *)(encrypt_key),
...
static int run_loop(n2n_edge_t * eee)
{
    ...
    while (keep_running) {
        ...
        if (numPurged > 0) {
            traceEvent(TRACE_NORMAL,
                       "Peer removed: pending=%u, operational=%u",
                       (unsigned int)hashed_peer_list_t_size(eee->pending_peers),
                       (unsigned int)hashed_peer_list_t_size(eee->known_peers));
        }
