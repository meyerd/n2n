 ...
 */
ssize_t n2n_parse_hex(uint8_t * keyBuf, size_t keyLen, const char * textKey,
        size_t textLen)
{
    ssize_t retval = 0;
    uint8_t * pout = keyBuf;
...
static int parseKeyLine(n2n_cipherspec_t * spec, const char * linein)
{
                ...
                if (0 == parseKeyLine(k, line)) {
                    if (k->valid_until > now) {
                        traceEvent(TRACE_INFO,
                                   " --> [%u] from %lu, until %lu, transform=%hu, data=%s\n",
                        ...
                        traceEvent(TRACE_INFO,
                                   " --X [%u] from %lu, until %lu, transform=%hu, data=%s\n",
                                   idx, k->valid_from, k->valid_until, k->t, k->opaque);
                    }
                } else {
