 ...
 *         |<------ encrypted ------>|
 */
static int transop_encode_twofish(n2n_trans_op_t * arg, uint8_t * outbuf,
        size_t out_len, const uint8_t * inbuf, size_t in_len)
{
    int len = -1;
    transop_tf_t * priv = (transop_tf_t *) arg->priv;
    ...
    if ((in_len + TRANSOP_TF_NONCE_SIZE) <= N2N_PKT_BUF_SIZE) {
        if ((in_len + TRANSOP_TF_NONCE_SIZE + TRANSOP_TF_SA_SIZE
                + TRANSOP_TF_VER_SIZE) <= out_len) {
            size_t idx = 0;
            sa_twofish_t * sa;
            size_t tx_sa_num = 0;
 ...
 *         |<------ encrypted ------>|
 */
static int transop_decode_twofish(n2n_trans_op_t * arg, uint8_t * outbuf,
        size_t out_len, const uint8_t * inbuf, size_t in_len)
{
    int len = 0;
    transop_tf_t * priv = (transop_tf_t *)arg->priv;
      ...
      && (in_len >= (TRANSOP_TF_VER_SIZE + TRANSOP_TF_SA_SIZE + TRANSOP_TF_NONCE_SIZE)) /* Has at least version, SA and nonce */) {
        ...
        if (N2N_TWOFISH_TRANSFORM_VERSION == tf_enc_ver) {
            ...
            if (sa_idx >= 0) {
                ...
                traceEvent(TRACE_DEBUG, "decode_twofish %lu with SA %lu.",
                        in_len, sa_rx, sa->sa_id );

                len = TwoFishDecryptRaw(
                            (void *) (inbuf + TRANSOP_TF_VER_SIZE + TRANSOP_TF_SA_SIZE),
                ...
                if (len > 0) {
                    ...
                    memcpy(outbuf, assembly + TRANSOP_TF_NONCE_SIZE, len);
                } else {
...
static int transop_addspec_twofish(n2n_trans_op_t * arg,
        ...
        if (sep) {
            ...
            if (pstat > 0) {
                ...
                traceEvent(TRACE_DEBUG,
                        "transop_addspec_twofish sa_id=%u data=%s.\n",
            ...
            traceEvent(TRACE_ERROR,
                    "transop_addspec_twofish : bad key data - missing '_'.\n");
        }
    } else {
...
static n2n_tostat_t transop_tick_twofish(n2n_trans_op_t * arg, time_t now)
{
    ...
    for (i = 0; i < priv->num_sa; ++i) {
        if (0 == validCipherSpec(&(priv->sa[i].spec), now)) {
            ...
            traceEvent(TRACE_INFO,
                    "transop_tf choosing tx_sa=%u (valid for %lu sec)",
                    priv->sa[i].sa_id, remaining);
            priv->tx_sa = i;
            found = 1;
            break;
        } else {
            traceEvent(TRACE_DEBUG,
                    "transop_tf tick rejecting sa=%u  %lu -> %lu",
    ...
    if (found == 0) {
        traceEvent(TRACE_INFO,
                "transop_tf no keys are currently valid. Keeping tx_sa=%u", priv->tx_sa);
    } else {
        r.can_tx = 1;
...
int transop_twofish_setup(n2n_trans_op_t * ttt, n2n_sa_t sa_num,
