...
static int try_forward(n2n_sn_t * sss, const n2n_common_t * cmn,
        const n2n_mac_t dstMac, const uint8_t * pktbuf, size_t pktsize);

static int try_broadcast(n2n_sn_t * sss, const n2n_common_t * cmn,
        const n2n_mac_t srcMac, const uint8_t * pktbuf, size_t pktsize);
 ...
 *  supernode. */
static int update_edge(n2n_sn_t * sss, const n2n_REGISTER_SUPER_t * reg,
        const n2n_community_t community, const n2n_sock_t * sender_sock,
        time_t now)
{
    macstr_t mac_buf;
    ...
    traceEvent(TRACE_DEBUG, "update_edge for %s [%s]",
               macaddr_str(mac_buf, reg->edgeMac), sock_to_cstr(sockbuf, sender_sock));
    ...
    if (NULL == scan) {
        ...
        int num_changes = 0;
        scan->timeout = reg->timeout;
        if (memcmp(community, scan->community_name, sizeof(n2n_community_t)) != 0) {
            memcpy(scan->community_name, community, sizeof(n2n_community_t) );
            num_changes++;
        }
        if (sock_equal(sender_sock, &(scan->sock)) != 0) {
            memcpy(&(scan->sock), sender_sock, sizeof(n2n_sock_t));
        ...
        if (scan->num_sockets == 1) {
            ...
            if (reg->aflags & N2N_AFLAGS_LOCAL_SOCKET) {
                if (sock_equal(&(reg->local_sock), scan->sockets+1) != 0) {
 ...
 */
static ssize_t sendto_sock(n2n_sn_t * sss, const n2n_sock_t * sock,
        const uint8_t * pktbuf, size_t pktsize)
{
    ...
    if (AF_INET == sock->family) {
        ...
        traceEvent(TRACE_DEBUG, "sendto_sock %lu to [%s]", pktsize, sock_to_cstr(sockbuf,sock));
        ...
        const n2n_mac_t dstMac, const uint8_t * pktbuf, size_t pktsize)
{
    ...
    if (NULL != scan) {
        ...
        if (data_sent_len == pktsize) {
            ++(sss->stats.fwd);
            traceEvent(TRACE_DEBUG, "unicast %lu to [%s] %s", pktsize,
        ...
        const n2n_mac_t srcMac, const uint8_t * pktbuf, size_t pktsize)
{
    ...
    for (ll = sglib_hashed_peer_info_t_it_init(&it, sss->edges); ll != NULL;
            ll = sglib_hashed_peer_info_t_it_next(&it)) {
        if ((memcmp(ll->community_name, cmn->community, sizeof(n2n_community_t)) == 0)
            ...
            /* REVISIT: exclude if the destination socket is where the packet came from. */
        {
            ...
            if (data_sent_len != pktsize) {
                ...
                traceEvent(TRACE_WARNING, "multicast %lu to [%s] %s failed %s",
                        pktsize, sock_to_cstr(sockbuf, &(ll->sock)),
                        macaddr_str(mac_buf, ll->mac_addr),
                        strerror(errno));
            } else {
                ++(sss->stats.broadcast);
                traceEvent(TRACE_DEBUG, "multicast %lu to [%s] %s", pktsize,
...
static int process_mgmt(n2n_sn_t * sss, const struct sockaddr_in * sender_sock,
        const uint8_t * mgmt_buf, size_t mgmt_size, time_t now)
{
 ...
 *
 */
static int process_udp(n2n_sn_t * sss, const struct sockaddr_in * sender_sock,
        const uint8_t * udp_buf, size_t udp_size, time_t now)
{
    ...
    case MSG_TYPE_QUERY_PEER:
        ...
        if (scan && memcmp(cmn.community, scan->community_name, sizeof(n2n_community_t)) == 0) {
    ...
    case MSG_TYPE_REGISTER:
        ...
        unicast = (is_multi_broadcast(reg.dstMac) == 0);
        ...
        if (unicast) {
            ...
            if ((cmn.flags & N2N_FLAGS_FROM_SUPERNODE) != 0) {
                memcpy(&cmn2, &cmn, sizeof(n2n_common_t));
    ...
    while (keep_running) {
        ...
        if (rc > 0) {
            if (FD_ISSET(sss->sock, &socket_mask)) {
                ...
                bread = recvfrom(sss->sock, pktbuf, N2N_SN_PKTBUF_SIZE,
                        0/*flags*/, (struct sockaddr *) &sender_sock,
                        (socklen_t*) &i);

                if (bread < 0) /* For UDP bread of zero just means no data (unlike TCP). */ {
                    /* The fd is no good now. Maybe we lost our interface. */
                    traceEvent(TRACE_ERROR,
                               "recvfrom() failed %d errno %d (%s)", bread, errno, strerror(errno));
                    keep_running=0;
                    break;
                }
            ...
            if (FD_ISSET(sss->mgmt_sock, &socket_mask)) {
                ...
                i = sizeof(sender_sock);
                bread = recvfrom(sss->mgmt_sock, pktbuf, N2N_SN_PKTBUF_SIZE,
                        0/*flags*/, (struct sockaddr *) &sender_sock,
                        (socklen_t*) &i);
