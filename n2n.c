...
SGLIB_DEFINE_HASHED_CONTAINER_FUNCTIONS(peer_info_t, PEER_HASH_TAB_SIZE,
...
SOCKET open_socket(int local_port, int bind_any)
{
    ...
    setsockopt(sock_fd, SOL_SOCKET, SO_REUSEADDR, (char *) &sockopt,
               sizeof(sockopt));

    memset(&local_address, 0, sizeof(local_address));
    local_address.sin_family = AF_INET;
    local_address.sin_port = htons(local_port);
    local_address.sin_addr.s_addr = htonl(
            bind_any ? INADDR_ANY : INADDR_LOOPBACK);
...
void print_n2n_version()
{
    printf("Welcome to n2n v.%s for %s\n"
            ...
            n2n_sw_osName, n2n_sw_buildDate);
}
...
size_t hashed_peer_list_t_size(peer_info_t** htab)
{
    ...
    for (ll = sglib_hashed_peer_info_t_it_init(&it, htab); ll != NULL; ll =
...
size_t purge_hashed_peer_list_t(peer_info_t ** peer_list, time_t purge_before)
{
    ...
    for (ll = sglib_hashed_peer_info_t_it_init(&it, peer_list); ll != NULL; ll =
            sglib_hashed_peer_info_t_it_next(&it)) {
